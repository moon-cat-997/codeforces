...
void solve() {
    ...
    unordered_set<char> uniquePokemons;
    rep(i, 0, sz(s)) {
        uniquePokemons.insert(s[i]);
    }
    int uniquePokemonsCnt = sz(uniquePokemons);
    map<char, int> pokemonsCountMap;
    int curUniquePokemons = 0;
    int bestLen = 100000;
    int lo = 0;
    rep(hi, 0, n) {
        if (pokemonsCountMap[s[hi]] == 0) {
            curUniquePokemons++;
        }
        pokemonsCountMap[s[hi]]++;
        while(lo < hi && pokemonsCountMap[s[lo]] > 1) {
            pokemonsCountMap[s[lo]]--;
            lo++;
        }
        if (curUniquePokemons == uniquePokemonsCnt) 
            bestLen = min(bestLen, hi - lo + 1);
