...
- **Current focus:** two pointers / sliding window (roadmap topic #4) — **all shapes now covered**: max-window ✓ (279B), converging ✓ (6C), sort-then-window ✓ (1133C, 580B), two-sequence matching ✓ (489B), offline sorted queries ✓ (600B), counting pairs by sum threshold ✓ (1324D), min-window ✓ (701C), distinct-count cap ✓ (616D)
- **In progress:** `1354B` Ternary String (1200, min-window — scaffolded, skipped for now). **Ladder A is complete** (10/10, capstone 616D done 2026-08-02).
- **Queued:** binary-search ladder (Ladder B) is scaffolded with official samples — see below. Order: finish 1354B → binary-search ladder.
